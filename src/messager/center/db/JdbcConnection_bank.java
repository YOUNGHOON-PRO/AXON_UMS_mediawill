...
public class JdbcConnection_bank
{
     ...
     */
    public static JdbcConnection_bank getWorkConnection()
        throws ClassNotFoundException, SQLException {

        synchronized (JdbcConnection_bank.class) {
        ...
        //db의 charset
        String dbCharsetName = (String) workDBProperties.get("bank.db.charset");

        // 컨텐츠의 charset
        String userCharsetName = (String) workDBProperties.get("bank.user.charset");

        //JDBC Driver
        String driverName = (String) workDBProperties.get("bank.jdbc.driver.name");

        // JDBC URL
        String url = (String) workDBProperties.get("bank.jdbc.url");

        // DB의 User Name
        String user = (String) workDBProperties.get("bank.db.user");

        // DB의 User Password
        String password = (String) workDBProperties.get("bank.db.password");

        Properties props = new Properties();

        if (user != null) {
            props.put("bank.user", user);
        }
        ...
        props.put("bank.password", password);
        //JDBC Driver 로드
        Class.forName(driverName);
        //JdbcConnection 객체를 생성한다.
        return new JdbcConnection_bank(url, dbCharsetName, userCharsetName, props);
    }
     ...
     *                SQLException
     */
    public static JdbcConnection_bank getInstance(Properties props)
        throws ClassNotFoundException, SQLException {
        ...
        Class.forName(driverName);
        //JdbcConnection객체 생성
        JdbcConnection_bank connection
            = new JdbcConnection_bank(url, dbCharsetName, userCharsetName, dbProps);
        // DB에 접속한다.
     ...
     */
    private JdbcConnection_bank(String url, String dbCharset, String userCharset,
                           Properties props)
        throws SQLException {
        jdbcUrl = url;
